...
class LLParser {
    ...
    function grammar_init($_raw_grammar) {
        ...
        foreach ($_raw_grammar as $nt => $rules) {
            ...
            foreach ($rules as $rule) {
                ...
                if ($rule === '') {
                    $rule = '<empty>';
                } //it is really a terminal symbol
    ...
    function find_close($str, $start, $closer) {
        ...
        while ($i < $cnt) {
            $before = $current;
            $current = $str[$i];
            if ($current == $closer && $before != '\\') {
                $level--;
            }
            if ($current == $opener && $before != '\\') {
                $level++;
            }
            if ($level == 0) {
                return $i;
    ...
    function add_left_follows($from, $to) {
        ...
        if (!isset($this->_follow[$from])) {
            $this->find_follow_term($from);
        }
    ...
    function parse($_str) {
        ...
        array_push($_stack, array('s' => ($t = $this->_grammar[0]['left']), 'k' => $_tree->go($_tree->add($t))));
        $limit = 0;
        for ($now = 0; $now < $cnt;) {
            if ($limit++ > 1000) {
                ...
                if ($this->isTerminal($st) || $this->_fsm_map[$st][$char_type] === 'none') {
                    if ($st !== '<empty>') {
                        ...
                        if ($tt == '') {
                            ...
                            $this->error .= 'Unexpected sequence "' . $tt . "\"\n";
                        }
                        return false;
                    }
                } else {
