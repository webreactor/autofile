...
class TagSystem {
    ...
    function try_parse($_tag, $_str) {
        if (!preg_match('/^\s*([a-zA-Z0-9_]+)(\s.+)?$/u', $_tag->open_raw, $preg_data)) {
            return $_tag;
        }
        ...
        if ($_ready_tag === false) {
            $this->compiler->error_in_tag('Cannot find closing tag ' . $_new_tag->get_closer(), $_new_tag);
            return $_tag;
        }
        return $_ready_tag;
    }
    ...
    function parse_end_of_tag($_tag, $_str) {
        ...
        preg_match_all('/' . $open_token . '\s*' . $_tag->name . '(\s.+)?' . $close_token . '/Usu', $_str, $opens, PREG_OFFSET_CAPTURE, $_tag->open_length);
        preg_match_all('/' . $open_token . '\s*' . $closer . '\s*' . $close_token . '/Uu', $_str, $closes, PREG_OFFSET_CAPTURE, $_tag->open_length);
        ...
        if ($f == 0) {
            $_tag->content_raw = mb_substr($_str, $_tag->open_length, $pos - $_tag->open_length);
