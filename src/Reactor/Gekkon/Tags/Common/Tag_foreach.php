...
class Tag_foreach extends BaseTag {
    function compile($compiler) {
        ...
        if ($args === false) {
            ...
            if ($args_data === false) {
                return $compiler->error_in_tag('Cannot parse args "' . $this->args_raw . '"', $this);
            }
            $args = $exp_compiler->parse_construction($args_data, array('<exp>', 'as', '<exp>', '=', '>', '<exp>', ';', '<exp>'), false);
            if ($args === false) {
                $args = $exp_compiler->parse_construction($args_data, array('<exp>', 'as', '<exp>', ';', '<exp>'), false);
            }
            if ($args !== false) {
                ...
                if (count($args) > 2) {
                    ...
                    $x_args = array('from' => $args[0], 'item' => $args[2],);
                    if (isset($args[5])) {
                        $x_args['key'] = $args[2];
                        $x_args['item'] = $args[5];
                    }
        ...
        if ($args === false) {
            ...
            $args = $exp_compiler->compile_construction_expressions($args);
            if (!isset($args['from'])) {
                return $compiler->error_in_tag('Missing required argument "from"', $this);
            }
            if (!isset($args['item'])) {
                return $compiler->error_in_tag('Missing required argument "item"', $this);
            }
            if ($args === false) {
                return $compiler->error_in_tag('Cannot compile args "' . $this->args_raw . '"', $this);
            }
        ...
        return $meta_init . $loop_start . $compiler->compile_parsed_str($content[0]) . $meta_body . "$_is_empty=false;}\n" . $empty;
    }
}
